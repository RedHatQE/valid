...
class ValidTestcase(object):
    ...
    def ping_pong(self, connection, command, expectation, timeout=10):
        ...
        try:
            ...
            logging.debug(multiprocessing.current_process().name + ": ping_pong failed: '%s'" % e.message)
    ...
    def match(self, connection, command, regexp, grouplist=[1], timeout=10):
        try:
            ...
            logging.debug(multiprocessing.current_process().name + ": match failed '%s'" % e.message)
            return None
    ...
    def get_result(self, connection, command, timeout=10):
        try:
            ...
            logging.debug(multiprocessing.current_process().name + ": getting failed: '%s'" % e.message)
            return None
