...
class ValidTestcase(object):
    ...
    def ping_pong(self, connection, command, expectation, timeout=10):
        ...
        logging.debug(multiprocessing.current_process().name + ": ping_pong '%s' expecting '%s'" % (command, expectation))
        result = {"command": command, "expectation": expectation}
        try:
            Expect.ping_pong(connection, command, expectation, timeout)
            result["result"] = "passed"
            logging.debug(multiprocessing.current_process().name + ": ping_pong passed")
        except ExpectFailed, err:
            result["result"] = "failed"
            result["actual"] = err.message
            logging.debug(multiprocessing.current_process().name + ": ping_pong failed: '%s'" % err.message)
    ...
    def match(self, connection, command, regexp, grouplist=[1], timeout=10):
        ...
        try:
            logging.debug(multiprocessing.current_process().name + ": matching '%s' against '%s'" % (command, regexp.pattern))
            ...
            logging.debug(multiprocessing.current_process().name + ": matched '%s'" % result)
            return result
        except ExpectFailed, err:
            self.log.append({"result": "failed", "match": regexp.pattern, "command": command, "actual": err.message})
            logging.debug(multiprocessing.current_process().name + ": match failed '%s'" % err.message)
            return None

    def get_result(self, connection, command, timeout=10):
        """ Expect.match wrapper """
        try:
            logging.debug(multiprocessing.current_process().name + ": getting result for '%s'" % command)
            ...
            logging.debug(multiprocessing.current_process().name + ": got result: '%s'" % result[0])
            return result[0]
        except ExpectFailed, err:
            self.log.append({"result": "failed", "command": command, "actual": err.message})
            logging.debug(multiprocessing.current_process().name + ": getting failed: '%s'" % err.message)
            return None

    def get_return_value(self, connection, command, timeout=15, expected_status=0, nolog=False):
        """ Connection.recv_exit_status wrapper """
        logging.debug(multiprocessing.current_process().name + ": getting return value '%s'" % command)
        ...
        logging.debug(multiprocessing.current_process().name + ": got '%s' status" % status)
        return status
