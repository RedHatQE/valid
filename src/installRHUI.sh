...
if [ "$server" == "rhua" ]; then
 if [ -e "/etc/pulp/pulp.conf" ]; then
  perl -npe 's/server_name: localhost/server_name: $rhua/g' -i /etc/pulp/pulp.conf;
  cat /etc/pulp/pulp.conf | grep server_name 
 fi
 if [ -e "/etc/pulp/client.conf" ]; then
  perl -npe 's/host = localhost.localdomain/host = $rhua/g' -i /etc/pulp/client.conf;
  cat /etc/pulp/client.conf | grep host
 fi
 if [ -e "/etc/pulp/consumer/consumer.conf" ]; then
  perl -npe 's/host = localhost.localdomain/host = $rhua/g' -i /etc/pulp/consumer/consumer.conf;
  cat /etc/pulp/consumer/consumer.conf | grep host
 fi
 if [ -e "/etc/rhui/rhui-tools.conf" ]; then
  perl -npe 's/hostname: localhost/hostname: $rhua/g' -i /etc/rhui/rhui-tools.conf;
...
if [[ "$server" == "cds1" ]] || [[ "$server" == "cds2" ]]; then
 perl -npe 's/host = localhost.localdomain/host = '${rhua}'/g' -i /etc/pulp/cds.conf;
