...
if [ "$server" == "rhua" ]; then
 if [ -e "/etc/pulp/pulp.conf" ]; then
  sed -i 's/server_name: localhost/server_name: $rhua/g' /etc/pulp/pulp.conf;
  cat /etc/pulp/pulp.conf | grep server_name 
 fi
 if [ -e "/etc/pulp/client.conf" ]; then
  sed -i 's/host = localhost.localdomain/host = $rhua/g' /etc/pulp/client.conf;
  cat /etc/pulp/client.conf | grep host
 fi
 if [ -e "/etc/pulp/consumer/consumer.conf" ]; then
  sed -i 's/host = localhost.localdomain/host = $rhua/g' /etc/pulp/consumer/consumer.conf;
  cat /etc/pulp/consumer/consumer.conf | grep host
 fi
 if [ -e "/etc/rhui/rhui-tools.conf" ]; then
  sed -i 's/hostname: localhost/hostname: $rhua/g' /etc/rhui/rhui-tools.conf;
